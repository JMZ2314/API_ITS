...
class CourseView(APIView,PageNumberPagination):
    ...
    def get (self,request):
        try:
            ...
            return Response({'success': True , 'count': len(courses_ordered)  , 'data': courses_with_progress if has_user else serializer.data, 'next':self.get_next_link() , 'previous':self.get_previous_link() , 'status:' : status.HTTP_200_OK }, status= status.HTTP_200_OK  )
